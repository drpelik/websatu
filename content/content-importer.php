...
class Theme_Content_Import {
	...
	private function insert_pages($pages_info) {
		...
		foreach ($pages_info as $num => $page) {
			// ignores Posts Page, if this is Front Page
			if (isset($page['posts_page']) && $page['posts_page'] && 0 == $num) {
				continue;
			}
			...
			if ($page['show_in_vmenu']) {
				$this->add_to_menu('vmenu',  $id, $parent_id, $page['title_in_menu']);
			}
			if ($page['show_in_menu']) {
				$this->add_to_menu('hmenu',  $id, $parent_id, $page['title_in_menu']);
			}
		...
		if (isset($this->inserted_ids[$pages_info[0]['path']])) {
			update_option('show_on_front', 'page');
			update_option('page_on_front', $this->inserted_ids[$pages_info[0]['path']]);
	...
	private function add_to_menu($type, $id, $parent_id, $title) {
...
add_action('switch_theme', 'theme_delete_option');
